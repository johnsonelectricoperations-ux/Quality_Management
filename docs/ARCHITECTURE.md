...
7-1. **Scrap Cost·COPQ 단가는 "불량을 발견한 공정(입력공정)" 기준**(2026-07-29 확정,
   `calc.Masters.scrap_price()`). 불량유형 마스터의 **배분규칙은 비용에 적용하지 않는다** —
   공정별 불량수량 분석에만 쓴다. (재배분하면 후처리에서 발견된 불량이 성형 0.5배 단가로
   계산돼 실패비용이 과소평가된다. 실제로 6월 1PART 비교 시 재배분 방식은 8,418천원,
   발견공정 방식은 12,046천원으로 레거시 엑셀 12,262천원에 근접했다.)
   · **외주소재불량**은 공정단가가 아니라 **완제품(기타)단가 × 파트별 배수**
     (`calc.OUTSOURCE_PRICE_RATIO` = 1PART 0.8 / 2PART 0.9, 2026-07-29 확정).
   · 그 결과 `process_breakdown()`의 공정별 표는 **수량=원인공정 배분, 비용=발견공정 기준**으로
     열마다 기준이 다르다.
7-2. **세부지표현황(`report_kpi.py`)의 성형/소결/정형/가공/기타 5행 배분은 "귀책(원인)공정"
   기준**(2026-07-29 확정, `report_kpi.month_process()`). 총액(합계) 계산은 7-1과 동일하게
   발견공정 단가를 그대로 쓰지만(총액 불변), **그 비용이 어느 행에 찍히는지는 불량유형 마스터의
   배분규칙(`Masters.resolve()`)을 따른다** — 즉 후처리 시트에서 발견된 불량이라도 그 불량유형의
   귀책이 성형이면 "성형" 행에 비용이 집계된다(단가는 발견공정=후처리 단가 그대로, 수량 배분
   비율만큼 비용도 비례배분). 배분으로 qty가 여러 버킷에 쪼개져도 총합은 정확히 보존된다.
   raw(종합) 섹션은 이 배분과 무관하게 **발견 그대로**(성형/소결/정형/가공/압입/후처리/외주소재/
   폐기불량 8종)를 보여준다 — "종합=어디서 발견됐나", "공정별=누가 원인이었나"로 성격이 다르다.
7-3. **세부지표현황의 "누계" 열**(2026-07-29 확정, `report_kpi.latest_actual_month()` /
   `_months_so_far()`). Scrap Cost·Quantity·COPQ·공정불량 탭의 맨 왼쪽 "누계" 열은 FY 12개월
   중 **생산량 데이터가 있는 마지막 달까지만** 합산한다(그 이후 달은 아직 미발생이므로 제외).
   %·ppm 같은 비율 지표는 월별 비율의 평균이 아니라 **누적 분자 ÷ 누적 분모**로 재계산한다
   (예: Scrap Cost 실적% 누계 = Σ(월별 불량비용) ÷ Σ(월별 SVP)). 공정불량 탭의 누계 "목표(ppm)"만
   예외로, ppm 목표는 원래 단순 합산 대상이 아니라서 **데이터가 있는 달들의 단순평균**을 쓴다
   (실적·비용·수량 지표처럼 자연스러운 "누적 분모"가 없기 때문 — 필요시 재검토).
   Warranty·Customer Incident 탭은 애초에 월별 열이 "그 달 발생분만"이라(7-4 참고) 누계 열이
   곧 FY 합계와 같다.
7-4. **Warranty·Customer Incident 탭의 월별 열은 "그 달 발생분만"**(2026-07-29 확정,
   `report_kpi._block_warranty()` / `_block_incident()`). 과거엔 FY 시작부터의 누적 합이었지만,
   7-3의 "누계" 열이 그 역할을 대신하게 되면서 월별 열은 단순 월간 수치로 바뀌었다.
7-5. **불량유형별 탭은 [item별]/[유형별] 두 서브탭**(2026-07-29 확정). item별은 기존 화면
   그대로(Part·TM-NO·단일 불량유형·기간, 일 단위). 유형별은 Part + 불량유형 최대 3개(복수 선택,
   색상으로 구분된 꺾은선) + 집계단위(일/주/월). 일=사용자가 지정한 기간, 월=최근 12개월(월
   라벨), 주=최근 7개월 범위 내 실제 존재하는 주차들을 "N월M주" 라벨로(`calc.week_of_month()`
   기준, 월요일~일요일). TM-NO 입력은 2글자 이상 앞자리(prefix) 일치로 자동완성되며(등록 안 된
   품번 포함, `defect_entry` 기준), Part를 먼저 선택하면 검색 범위가 그 파트로 좁혀진다. TM-NO를
   선택하면 불량유형 드롭다운이 그 품번에 실제 발생한 유형만으로 좁혀진다.
8. **공정별 단가(성형/소결/정형/가공/기타) 산출 규칙**(2026-07-28 확정, `app/price_calc.py`):
   ...
   - **1PART(VMS PART)**: 정형공정이 있는 품목은 성형=나머지×0.5, 소결=나머지×0.6, 정형=나머지×0.8.
     정형공정이 없는 품목은 성형=나머지×0.6, 소결=나머지×0.8.
   - **2PART(TM PART)**: 정형 유무와 무관하게 **성형=나머지×0.5, 소결=나머지×0.8 고정**
     (정형이 있으면 정형=나머지×0.8도 추가) — 2026-07-29 확정, 기존 엑셀 실적과 대조해 결정.
     `price_calc.ratios_for(part, has_jeonghyeong)` 가 파트별 배수를 반환한다.
   - **가공·기타는 항상 나머지공정단가 그대로**(둘이 같은 값, 파트 공통).
